...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    uint256 public token0Lock = 0;
    uint256 public take0Take = 0;
    uint256 public token1Lock = 0;
    uint256 public take1Take = 0;
    ...
    function initialize(address _token0, address _token1, address _controller) external {
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        // token lock
        uint256 lock0 = balance0.mul(lockPercent).div(100);
        if (lock0 > token0Lock) {
            token0Lock = lock0;
        }
        uint256 lock1 = balance1.mul(lockPercent).div(100);
        if (lock0 > token0Lock) {
            token1Lock = lock1;
        }
        balance0 = balance0.add(token0Take);
        balance1 = balance1.add(token1Take);
    ...
    // take token from pair
    function takeToken(uint256 tokenType, address to, uint256 amount) public {
        require(msg.sender == controller, "Only controller can take this");
        if (tokenType == 0) {
            require(amount <= token0Lock, "Over token0 number lock");
            token0Take = token0Take.add(amount);
            _safeTransfer(token0, to, amount);
        } else if (tokenType == 1) {
            require(amount <= token0Lock, "Over token0 number lock");
            token1Take = token1Take.add(amount);
            _safeTransfer(token1, to, amount);
        }
    }

    // input token to pair
    function inputToken(uint256 tokenType, address to, uint256 amount) public {
        require(msg.sender == controller, "Only controller can take this");
        if (tokenType == 0) {
            token0Take = token0Take.sub(amount);
            IERC20(token0).transferFrom(msg.sender, address(this), amount);
        } else if (tokenType == 1) {
            token1Take = token1Take.sub(amount);
            IERC20(token0).transferFrom(msg.sender, address(this), amount);
        }
    }
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            balance1 = IERC20(_token1).balanceOf(address(this));
            // token lock
            require(balance0 > token0Lock, 'Pancake: EXCEED TOKEN0 LOCK VALUE');
            require(balance1 > token1Lock, 'Pancake: EXCEED TOKEN1 LOCK VALUE');
            balance0 = balance0.add(token0Take);
            balance1 = balance1.add(token1Take);
            uint256 lock0 = balance0.mul(lockPercent).div(100);
            if (lock0 > token0Lock) {
...
contract PancakeFactory is IPancakeFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        IPancakePair(pair).initialize(token0, token1, admin);
